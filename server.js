...
app.post('/generate-pdf', (req, res) => {
    ...
    doc.fillColor('#334155').font('Helvetica-Bold').fontSize(12).text('2. TUBAZIONI E TRACCIATO CAD', 50, 240);
    doc.font('Helvetica').fontSize(10).fillColor('#000000');
    doc.text(`Materiale: ${data.material} - ${data.diameter}`, 50, 260);
    doc.text(`Normativa: ${data.standard}`, 50, 280);
    doc.font('Helvetica-Bold').fillColor('#3b82f6').text(`Metratura Totale Rilevata CAD: ${data.metratura} metri`, 50, 300);
    
    doc.fillColor('#334155').font('Helvetica-Bold').fontSize(12).text('3. POSA AEREA', 50, 340);
    doc.font('Helvetica').fontSize(10).fillColor('#000000');
    doc.text(`Accesso: ${data.aereaAccesso} | Zincatura: ${data.aereaZincatura}`, 50, 360);
